 ...
 */
@Component
public class StartPrimaryElectionCommandHandler extends TelegramCommandHandler {

	@Override
	protected Collection<String> getCommandString() {
		return Collections.singleton(Constants.Command.START_ELECTION);
	}
	...
	public void execute(TMessage message) {
		...
		client.send(new SendMessageWithReplyKeyboard()
				...
				.setText(MessageHolder.get("election.started", room.getLang()))
		);
	}
}
